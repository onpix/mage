...
class DownBlock(nn.Module):
    def __init__(
        ...
        for i in range(num_layers):
            ...
            if attention:
                attns.append(
                    MVAttention(out_channels, attention_heads, skip_scale=skip_scale, num_frames=num_frames)
                )
            else:
                attns.append(None)
...
class MidBlock(nn.Module):
    def __init__(
        ...
        for i in range(num_layers):
            ...
            if attention:
                attns.append(
                    MVAttention(in_channels, attention_heads, skip_scale=skip_scale, num_frames=num_frames)
                )
            else:
                attns.append(None)
        self.nets = nn.ModuleList(nets)
...
class UNet(nn.Module):
    def __init__(
        ...
        if self.opt.ref_maps is not None:
            self.ref_in_channels = 3 * len(self.opt.ref_maps)
            self.conv_in_ref = nn.Conv2d(
                self.ref_in_channels,
                down_channels[0],
                kernel_size=3,
                stride=1,
                padding=1,
            )
            # # initialize self.conv_in_ref with very small weights and bias
            # torch.nn.init.xavier_normal_(self.conv_in_ref.weight, gain=1e-2)
            torch.nn.init.constant_(self.conv_in_ref.weight, 0)
            torch.nn.init.constant_(self.conv_in_ref.bias, 0)
            # print(f'weight after init: {self.conv_in_ref.weight.sum()}')
            # print(f'weight after init: {self.conv_in_ref.weight.max()}')
            # print(f'weight after init: {self.conv_in_ref.weight.min()}')
            # print(f'bias after init: {self.conv_in_ref.bias.sum()}')
            # print(f'bias after init: {self.conv_in_ref.bias.max()}')
            # print(f'bias after init: {self.conv_in_ref.bias.min()}')
            # exit()
        ...
        self.mid_block = MidBlock(
            down_channels[-1], attention=mid_attention, skip_scale=skip_scale, num_frames=self.opt.num_input_views
        )
    ...
    def forward(self, x):
        ...
        if self.opt.ref_maps is not None:
            ...
            # image + ray embedding
            x = x_in[
                :, : self.opt.unet_in_channel, :, :
            ]  # first 9 channels for rgb + ray embedding
            x = self.conv_in(x)

            # ref maps
            x_ref = x_in[:, self.opt.unet_in_channel :, :, :]
            assert x_ref.shape[1] == self.ref_in_channels
            x_ref = self.conv_in_ref(x_ref)
            x = x + x_ref
