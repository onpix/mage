    ...
    def shade(self, gb_pos, gb_normal, kd, ks, view_pos, specular=True):
        ...
        if self.mtx is not None:  # Rotate lookup
            ...
            reflvec = ru.xfm_vectors(
                reflvec.view(
                    reflvec.shape[0], reflvec.shape[1] * reflvec.shape[2], reflvec.shape[3]
                ),
                mtx,
            ).view(*reflvec.shape)
        ...
        diffuse = dr.texture(
            self.diffuse[None, ...], nrmvec.contiguous(), filter_mode="linear", boundary_mode="cube"
        )
        ...
        if specular:
            ...
            if not hasattr(self, "_FG_LUT"):
                self._FG_LUT = torch.as_tensor(
                    np.fromfile("render_loss/data/irrmaps/bsdf_256_256.bin", dtype=np.float32).reshape(
                        1, 256, 256, 2
                    ),
                    dtype=torch.float32,
                    device="cuda",
        ...
        #
        # renering without hemisphere visibility (leave channel of ks[0] unused)
        # import ipdb; ipdb.set_trace()
        return shaded_col
